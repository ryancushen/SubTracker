...
class SubscriptionListItem(ListItem):
    ...
    def __init__(self, subscription: Subscription):
        ...
        self.item_label = Label(f"{subscription.name} (${subscription.price:.2f} - Next: {renewal_info})")
    ...
    def _refresh_subscription_list(self) -> None:
        ...
        try:
            ...
            if not subs:
                ...
                # For now, the list will just be empty.
                self.log("No subscriptions found.")
                # Consider adding a Static widget placeholder if desired
            else:
                self.log(f"Populating list with {len(subs)} subscriptions.")
                for sub in subs:
    ...
    def _refresh_right_pane(self) -> None:
        ...
        if self.calendar:
            try:
                ...
                new_highlighted_dates: Set[date] = set()
                for sub, desc in events_raw:
                    if sub.next_renewal_date:
                        new_highlighted_dates.add(sub.next_renewal_date)
    ...
    def _on_add_dialog_closed(self, data: Optional[Dict[str, Any]]) -> None:
        ...
        if data:
            ...
            try:
                ...
                error_msg = f"Failed to add subscription: {e}"
    ...
    def _on_edit_dialog_closed(self, data: Optional[Dict[str, Any]]) -> None:
        ...
        if data:
            ...
            try:
                ...
                error_msg = f"Failed to update subscription: {e}"
                self.log.error(error_msg)
                self.notify(error_msg, title="Error", severity="error")
        else:
            ...
            if sub_id_to_delete is not None:
                ...
                try:
                    ...
                    if self.service.delete_subscription(sub_id_to_delete):
                        ...
                        self._refresh_all_data() # Refresh even on failure
                except Exception as e:
                    error_msg = f"Failed to delete subscription ID {sub_id_to_delete}: {e}"
                    self.log.error(error_msg)
                    self.notify(error_msg, title="Error", severity="error")
            else:
    ...
    def action_delete_sub(self) -> None:
        ...
        if self.selected_subscription_id is not None:
            ...
            try:
                ...
                # Error fetching the subscription to get its name for the dialog
                error_msg = f"Error preparing delete confirmation for ID {sub_id}: {e}"
                self.log.error(error_msg)
                self.notify(error_msg, title="Error", severity="error")
        else:
